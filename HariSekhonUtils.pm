...
BEGIN {
    # needs to be before die_sub(), otherwise could get 'Use of uninitialized value $nholuongutUtils::ERRORS{"CRITICAL"} in exit' and exit with blank / 0 incorrect error code on early stage failures such as 'This Perl not built to support threads'
    #
...
our $nagios_plugins_support_msg = "Please try latest version from https://github.com/nholuongut/Nagios-Plugins, re-run on command line with -vvv and if problem persists paste full output from -vvv mode in to a ticket requesting a fix/update at https://github.com/nholuongut/Nagios-Plugins/issues/new";
...
sub usage (;@) {
    ...
    if(not @_ and $main::DESCRIPTION){
        print STDERR "nholuongut - https://github.com/nholuongut";
        if($github_repo){
...
sub version_string () {
    ...
    $version_str .= "nholuongut Utils version $nholuongutUtils::VERSION";
    return $version_str;
}
